  ...
  getMachines()
  {
    ...
    this._CurrentActivityService.machinesValues.subscribe((res) => {
      ...
      data.forEach((element,i) => {
        this.data1.push({x:element.timeStamp,y:parseInt((element.oee*100).toFixed(0))})
        this.data2.push({x:element.timeStamp,y:parseInt((element.avalability*100).toFixed(0))})
        this.data3.push({x:element.timeStamp,y:parseInt((element.performance*100).toFixed(0))})
        this.data4.push({x:element.timeStamp,y:parseInt((element.quality*100).toFixed(0))})
      });
      this.line.refresh();
